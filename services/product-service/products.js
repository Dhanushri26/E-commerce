  ...
  try {
      ...
      try {
        data = await docClient.send(
          new QueryCommand({
            ...
            FilterExpression:
              'attribute_not_exists(isDeleted)' + (!canViewB2bProducts(userContext) ? ' AND is_b2b_only = :b2b' : ''),
    ...
    if (method === 'PUT' && path.startsWith('/products/')) {
      ...
      if (!ownsProduct(userContext, existing))
        return createErrorResponse(403, 'Permission mismatch on operation target');
    ...
    if (method === 'DELETE' && path.startsWith('/products/')) {
      ...
      await docClient.send(
        new UpdateCommand({
          ...
          Key: { PK: pk, SK: sk },
          UpdateExpression:
            'SET isDeleted = :d, deletedAt = :now, deletedBy = :uid, updatedAt = :now, updatedBy = :uid',
          ExpressionAttributeValues: {
            ':d': true,
