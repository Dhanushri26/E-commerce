import { QueryCommand, PutCommand, GetCommand, UpdateCommand, BatchWriteCommand } from '@aws-sdk/lib-dynamodb';
import {
  buildResponse,
  createErrorResponse,
  createAuditFields,
...
const fetchProductDetails = async (docClient, productTable, productId) => {
...
const fetchInventoryDetails = async (docClient, inventoryTable, productId) => {
  const res = await docClient.send(
    new GetCommand({
...
export const handler = async (event) => {
  ...
  try {
    ...
    const { docClient, cartTable, productTable, inventoryTable } = getDbClient();
    ...
    if (method === 'POST' && path === '/cart/items') {
      ...
      const product = await fetchProductDetails(docClient, productTable, productId);
      ...
      const inventory = await fetchInventoryDetails(docClient, inventoryTable, productId);
      ...
      if (stockAvailable < finalQuantity) {
        return createErrorResponse(409, 'Requested item volume exceeds current available stock parameters', {
          stockAvailable,
        });
      }
    ...
    if (method === 'DELETE' && path.startsWith('/cart/items/')) {
      ...
      const targetKey = { PK: cartPartition, SK: `ITEM#${productId}` };
      const currentRes = await docClient.send(new GetCommand({ TableName: cartTable, Key: targetKey }));
      if (!currentRes.Item || currentRes.Item.isDeleted === true)
        return createErrorResponse(404, 'Targeted item not found');
