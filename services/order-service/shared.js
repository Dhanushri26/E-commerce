...
export const extractUserContext = (event) => {
  ...
  // User ID
  const userId = claims.sub || headers['x-user-id'] || headers['X-User-Id'] || 'anonymous';

  // Role
  let role = headers['x-user-role'] || headers['X-User-Role'] || 'Customer';
  ...
  // Business ID
  const businessId = claims['custom:businessId'] || headers['x-business-id'] || headers['X-Business-Id'] || null;
  ...
  return {
    ...
    taxExempt: String(headers['x-tax-exempt'] || 'false').toLowerCase() === 'true',
