...
export function SearchBar({ products = [], className, onNavigate }) {
  ...
  const suggestions = useMemo(() => {
    ...
    return products
      .filter((p) => `${p.name ?? p.title} ${p.category ?? ''} ${p.description ?? ''}`.toLowerCase().includes(q))
      .slice(0, 6);
  }, [query, products]);
        ...
        <div className="absolute left-0 right-0 top-full z-50 mt-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
          {query.trim() && suggestions.length > 0 && (
            <div className="border-b border-slate-100 p-2">
              <p className="px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">Suggestions</p>
          ...
          {query.trim() && suggestions.length === 0 && (
            <div className="p-6 text-center text-sm text-slate-500">No products found for &ldquo;{query}&rdquo;</div>
          )}
