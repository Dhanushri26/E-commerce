...
export default function InventoryPanel() {
  ...
  async function loadInventory() {
    ...
    try {
      const [inventoryResponse, productsResponse] = await Promise.all([getInventory(), getProducts()]);
  ...
  async function handleSyncInventory() {
    try {
      ...
      const existingInventory = new Set(inventory.map((item) => item.productId));
  ...
  const filteredInventory = useMemo(() => {
    return inventory.filter((item) => (item.productId || '').toLowerCase().includes(search.toLowerCase()));
  }, [inventory, search]);
          ...
          <tbody>
            {loading ? (
              ...
              filteredInventory.map((item) => (
                <tr key={item.inventoryId} className="border-t border-stone-800 hover:bg-stone-800/40">
                  <td className="px-6 py-4 font-mono text-sm">{item.product.title ? item.product.title : 'N/A'}</td>
                  ...
                  <td className="px-6 py-4">
                    <div title="In Stock">
                      {item.inventoryStatus === 'IN_STOCK' && <CircleCheck className="text-green-500" size={20} />}
                    </div>
                    <div title="Low Stock">
                      {item.inventoryStatus === 'LOW_STOCK' && <TriangleAlert className="text-yellow-500" size={20} />}
                    </div>
                    <div title="Out of Stock">
                      {item.inventoryStatus === 'OUT_OF_STOCK' && <CircleX className="text-red-500" size={20} />}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <button
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/60">
          <div className="w-[420px] rounded-2xl bg-stone-900 p-6">
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setEditingItem(null)} className="rounded-lg bg-stone-700 px-5 py-2">
                Cancel
              </button>

              <button onClick={saveInventory} className="rounded-lg bg-amber-500 px-5 py-2 text-black">
                Save
              </button>
            </div>
          </div>
