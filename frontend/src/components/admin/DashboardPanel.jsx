import { BarChart3, Boxes, CreditCard, Loader2, ShoppingBag } from 'lucide-react';

export default function DashboardPanel({ loading, stats, recentOrders, lowStockItems }) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.label} className="rounded-[1.25rem] border border-stone-800 bg-stone-900 p-5">
          ...
          <div className="space-y-3 text-sm">
            {loading ? (
              ...
              recentOrders.map((order) => (
                <div key={order.orderId} className="rounded-2xl bg-stone-800 px-4 py-3">
                  <div className="flex justify-between">
                    <span className="font-mono text-xs text-stone-300">
                      #{order.orderId?.substring(0, 8).toUpperCase()}
                    </span>

                    <span className="text-xs">₹{Number(order.totalAmount || 0).toLocaleString('en-IN')}</span>
                  </div>
