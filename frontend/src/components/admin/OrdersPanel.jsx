...
export default function OrdersPanel() {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order) => (order.orderId || '').toLowerCase().includes(search.toLowerCase()));
  }, [orders, search]);
          ...
          <tbody>
            {loading ? (
              ...
              filteredOrders.map((order) => (
                <tr key={order.orderId} className="border-t border-stone-800 hover:bg-stone-800/40">
                  <td className="px-6 py-4 font-mono text-sm">
                    {order.orderId ? `#${order.orderId.substring(0, 8).toUpperCase()}` : 'Missing info'}
                  </td>
                  ...
                  <td className="px-6 py-4">₹{Number(order.totalAmount || 0).toLocaleString('en-IN')}</td>
