  ...
  try {
    ...
    if (method === "GET" && path === "/products") {
      ...
      const data = await docClient.send(new QueryCommand({
        TableName: tableName,
        IndexName: "SK-index", // Optional: Querying via an SK-driven GSI is faster than scanning
        KeyConditionExpression: "SK = :sk",
        FilterExpression: "attribute_not_exists(isDeleted)" + (!canViewB2bProducts(userContext) ? " AND is_b2b_only = :b2b" : ""),
        ExpressionAttributeValues: { 
          ":sk": "METADATA",
          ...(!canViewB2bProducts(userContext) && { ":b2b": false })
        }
      }).catch(() => docClient.send(scanParams))); // Safe fallback if GSI isn't built yet
